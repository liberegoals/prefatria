...
Trapezi.prototype.processEnergiaΔΙΑΝΟΜΗ = function(energia) {
	...
	// τη διανομή.
	if (Server) this.trapeziFilaSaveSet();
	return this;
};
...
Trapezi.prototype.processEnergiaΔΗΛΩΣΗ = function(energia) {
	...
	agora = null;
	if (this.alast && (this.apasoCount === 2)) agora = true;
	else if ((this.apasoCount === 1) && this.tagrafo && this.alast.dilosiOxiTagrafo()) agora = true;
	else if (!this.anext) agora = true;
...
Trapezi.prototype.energiaDilosiEpomenosSet = function(level) {
	...
	else if (++level > 2) return this.partidaEpomenosSet();

	this.partidaEpomenosSet(epomenos = this.partidaEpomenosGet().epomeniThesi());
	if (this.apaso[epomenos]) return this.energiaDilosiEpomenosSet(level);
	if (epomenos === this.tagrafo) return this.energiaDilosiEpomenosSet(level);
	return this;
};
...
Trapezi.prototype.processEnergiaΤΖΟΓΟΣ = function(energia) {
	...
	// Στα φύλλα του τζογαδόρου έχουν προστεθεί τα φύλλα
	// του τζόγου.
	if (Server) this.trapeziFilaSaveSet(tzogadoros);
	return this;
};
...
Trapezi.prototype.energiaFiloBaza = function() {
	...
	if (this.partidaBazaCountGet() < 10) return this;
...
Trapezi.prototype.processEnergiaCLAIM = function(energia) {
	...
	if (this.sdilosi[epomenos].simetoxiIsPaso()) epomenos = epomenos.epomeniThesi();
	this.partidaEpomenosSet(epomenos);
	this.claim = [];
...
Trapezi.prototype.processEnergiaΠΑΡΑΙΤΗΣΗ = function(energia) {
	...
	if (pektis === this.partidaTzogadorosGet()) return this.processEnergiaParetisiPliromi(energia);
	if (this.sdilosi[pektis].simetoxiIsPaso()) return this.processEnergiaParetisiPliromi(energia);
